...
class LabelTest(TembaTest):
    ...
    def test_delete_folder(self):
        ...
        self.assertFalse(Label.user_objects.filter(pk=folder1.pk).exists())

        # check that contained labels are also deleted
        self.assertEqual(Label.user_objects.filter(pk__in=[label1.pk, label2.pk]).count(), 0)
        ...
        self.assertFalse(Label.user_objects.filter(pk=label3.pk).exists())
